...
def get_mAP(Yolo, dataset, score_threshold=0.60, iou_threshold=0.50, TEST_INPUT_SIZE=TEST_INPUT_SIZE):
...
if __name__ == '__main__':       
    if YOLO_FRAMEWORK == "tf": # TensorFlow detection
        if YOLO_TYPE == "yolov4":
            ...
            TRAIN_MODEL_NAME = TRAIN_MODEL_NAME + '_Tiny' if TRAIN_YOLO_TINY else YOLO_V4_WEIGHTS
        if YOLO_TYPE == "yolov3":
            Darknet_weights = YOLO_V3_TINY_WEIGHTS
            TRAIN_MODEL_NAME = TRAIN_MODEL_NAME + '_Tiny' if TRAIN_YOLO_TINY else YOLO_V3_WEIGHTS
    ...
    get_mAP(yolo, testset, score_threshold=0.60, iou_threshold=0.50, TEST_INPUT_SIZE=YOLO_INPUT_SIZE)
